...
export async function renderHowItWorks(container) {
  ...
  container.append(section('Haftarot are scored too, capped below the hardest Torah reading',
    ...
    p('What\'s left is scored the same way: length in words (40% -- judged against other haftarot, not Torah aliyot, since haftarot run far longer on average), vocabulary (30% -- weighted higher here, since unfamiliar words are most of what remains to trip over), trope (20%) and repetition (10%). Word rarity is judged against the Torah plus Nevi\'im together, because judging prophetic vocabulary against the Chumash alone would make ordinary Nevi\'im words look far rarer than they really are to someone who reads haftarot.'),
    p('Haftarot land on the exact same 1-10 scale as every Torah reading, then the result is weighted down by a flat 30%. They\'re genuinely easier than the equivalent Torah reading, but not trivial -- haftarah trope is a separate melody you have to learn, and they aren\'t short (the median is 324 words, against 195 for a typical aliyah; Shirat Devorah runs to 744) -- so a hard haftarah still scores in the mid-to-upper range, just short of what the hardest Torah aliyah would reach. Only the Ashkenazi reading is scored so far.'),
    p('When a parsha\'s haftarah is the longest or shortest within a tradition, it\'s flagged with a small "Longest" / "Shortest" tag right on that nusach\'s card. This is measured by word count alone, so it works for the Sefardi and Chabad readings too even though only Ashkenazi has a full difficulty score -- the three traditions genuinely read different-length text in a number of places (Beshalach\'s Ashkenazi and Chabad haftarah, Shirat Devorah, is the longest of any tradition; Chukat\'s Sefardi reading is the longest within Sefardi custom specifically, despite being unremarkable in Ashkenazi terms).'),
  ));
  ...
  container.append(section('The whole-parsha score is measured, not averaged',
    p('The five bars on a parsha\'s page rate the reading as a whole, and each one is measured against the other parshiot directly -- not by averaging its aliyot\'s individual scores. Averaging hid real differences: it rewarded a parsha whose aliyot are all evenly long over one that is genuinely longer overall but front-loads that length into one huge aliyah. Matot-Masei is the longest reading in the Torah, yet averaging its aliyah scores ranked it fractionally behind Chukat-Balak.'),
    p('Each criterion is anchored so the median parsha sits at the middle of the scale, with the two halves stretched independently to reach a real 1 and a real 10. That keeps the middle honest while letting genuine outliers show as outliers -- before this, Gotchas never went above 6.5 for any parsha, so the criterion looked far more uniform than it is. Anchoring on the median rather than just the extremes also means one unusual parsha stretches only its own half of the range, leaving everything else\'s real spacing intact.'),
    p('The overall score at the top of the page is then the ordinary weighted blend of those five, and -- unlike the individual criteria -- it is deliberately not stretched to fill the scale. Nothing scores a perfect 10 overall, and that is the point: a reading is almost never maximally hard on all five counts at once. Matot-Masei tops the list at about 8, on a maxed-out length and a high Gotchas score, while being easy on trope and highly repetitive. Forcing that to a 10 would claim something about it that simply is not true, and would mean the top score only ever told you which parsha ranked first rather than how hard it actually is.'),
  ));
